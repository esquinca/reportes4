...
var Configuration_table_responsive_checkbox_move_viatic_n4= {
  "order": [[ 8, "desc" ]],
  "select": true,
...
function viatics_table(datajson, table){
  ...
  var vartable = table.dataTable(Configuration_table_responsive_checkbox_move_viatic_n4);
  ...
  vartable.fnAddData([
    ...
    priority_s,
    status.usuario,
    '<a href="javascript:void(0);" onclick="enviar(this)" value="'+status.id+'" class="btn btn-default btn-sm" role="button" data-target="#modal-concept"><span class="fa fa-eye"></span></a>',
    status.estado
    ]);
  });
}
...
function enviar(e){
  ...
  $.ajax({
      type: "POST",
      url: "/view_pertain_viatic_ur_n4",
      data: { date_to_search: fecha, viatic : valor , _token : _token },
      success: function (data){
        if (data === "0") { // NO SE NECESITA APROBAR PAGO - ESTATUS APRUEBA
          console.log('NO SE NECESITA APROBAR PAGO - ESTATUS APRUEBA');
        ...
        if (data === "1") { //NECESITA APROBAR PAGO - ESTATUS APRUEBA
          console.log('NECESITA APROBAR PAGO - ESTATUS APRUEBA');
          cabecera_viatic(valor, _token);
          cuerpo_viatic(valor, _token);
          timeline(valor, _token);
          totales_concept_zsa(valor, _token);
